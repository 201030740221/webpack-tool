...
ExtractTextPlugin.prototype.renderExtractedChunk = function(chunk, templateParams) {
	...
	chunk.modules.forEach(function(module) {
		...
		if(templateParams){
			var data = {}
			var _entryArray = module._originalModule.context.split('/');
			var dataFile = path.join( module._originalModule.context , _entryArray[_entryArray.length-1] + ".json")
			if(fs.existsSync(dataFile)){
				data = JSON.parse(fs.readFileSync(dataFile));
			}

			data = assign(templateParams,data);
		
			moduleSource._value = ejs.render(moduleSource._value,data,{
				filename: path.join( module._originalModule.context, _entryArray[_entryArray.length-1] + ".html" )
			});
		}
		
		//console.log(moduleSource._value);

		//moduleSource._value = artTemplate.compile(moduleSource._value)(data).toString();
		...
		compilation.plugin("additional-assets", function(callback) {
			extractedChunks.forEach(function(extractedChunk) {
				if(extractedChunk.modules.length) {
					...
					if(self.options.chunksEntry[fileName]){
						pageChunks = ["common", fileName ]
						...
						var templateParams = {
							_assets:{
								...
								htmlWebpackPlugin: {
								  files: self.htmlWebpackPluginAssets(compilation, webpackStatsJson, pageChunks, self.options.excludeChunks),
								  options: self.options,
								}
							}
				        };
						
						source = this.renderExtractedChunk(extractedChunk,templateParams);
...
ExtractTextPlugin.prototype.htmlWebpackPluginAssets = function(compilation, webpackStatsJson, includedChunks, excludedChunks) {
  var self = this;
