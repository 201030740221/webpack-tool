...
module.exports.pitch = function(request) {
	...
	if(this[__dirname] === undefined) {
		throw new Error(
			'"extract-html-webpack-plugin" loader is used without the corresponding plugin, ' +
			'refer to https://github.com/webpack/extract-html-webpack-plugin for the usage example'
		);
		...
		var resultSource;
		if(query.remove) {
			resultSource = "// removed by extract-html-webpack-plugin";
		} else {
			resultSource = undefined;
		}

		if(query.extract !== false) {
			var childFilename = "extract-html-webpack-plugin-output-filename"; // eslint-disable-line no-path-concat
			var publicPath = typeof query.publicPath === "string" ? query.publicPath : this._compilation.outputOptions.publicPath;
			var outputOptions = {
				filename: childFilename,
				publicPath: publicPath
			};
			var childCompiler = this._compilation.createChildCompiler("extract-html-webpack-plugin", outputOptions);
